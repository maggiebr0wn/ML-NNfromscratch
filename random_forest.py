...
class RandomForest(object):
    ...
    def _bootstrapping(self, num_training, num_features, random_seed = None):
        ...
        row_id = np.arange(num_training)

        get_size = len(row_id)
        row_idx = np.random.choice(row_id, size = get_size, replace = True)

        # select features at random
        feature_id = np.arange(num_features)

        get_feat_size = self.max_features * len(feature_id)
        col_idx = np.random.choice(feature_id, size = int(get_feat_size), replace = False)
    ...
    def fit(self, X, y):
        ...
        while i < self.n_estimators:
            #print(i)
            # get X ind/col
            X_row_id = self.bootstraps_row_indices[i]
            X_col_id = self.feature_indices[i]
            sub_X = X[X_row_id][:, X_col_id]

            # labs
            # y_row_id = self.bootstraps_row_indices[i]
            sub_y = y[X_row_id]           

            # train
            (self.decision_trees[i]).fit(sub_X, sub_y)
    ...
    def plot_feature_importance(self, random_forest):
        ...
        X = random_forest.iloc[:, :-1]
        y = random_forest.iloc[:, -1]
        ...
        while i < self.n_estimators:
            ...
            labs = []
            for j in self.feature_indices[i]:
                labs.append(random_forest.columns[j])

            map_labs = {}
            for j in range(len(importances)):
                map_labs[labs[j]] = importances[j]

            sortedmap_labs = {k: v for k, v in sorted(map_labs.items(), key=lambda item: item[1], reverse = True)}

            sorted_labs = []
            sorted_imps = []
            for j in sortedmap_labs.keys():
                sorted_labs.append(j)
                sorted_imps.append(sortedmap_labs[j])


            fig = plt.figure()
            ax = fig.add_axes([0,0,1,1])
            ax.bar(sorted_labs, sorted_imps)
            plt.show()
